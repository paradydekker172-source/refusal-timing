...
层 1 (已完成): 文档诚实修订, 不改代码不重跑。
层 2 (已完成 2026-06-19, 见下): 修代码 8 项 + 重跑受影响实验。

---

# 层 2 执行记录 (2026-06-19): 修代码 + 重跑

8 项修复全部落地, 受影响实验全部重跑。原 buggy 脚本保留 (可复现对比), 修复版
为 `*_v2_*.py` + `weight_common.py` (权重侧共享修正层)。

## 代码修复 (8/8)

| # | 修复 | 文件 | 状态 |
|---|---|---|---|
| 1 | `_danger_kw_indicator` token 索引→真分词窗口 | analysis.py | ✓ |
| 2 | `np.gradient` 传真实 positions (非均匀采样) | measure.py, analysis.py | ✓ |
| 3 | H 与 λ 解耦: 样本<k_min 返回 NaN (不再钉 0) + log(k) 归一 | measure.py, neutral_measure.py | ✓ |
| 4 | LR 检验加 cluster bootstrap (按 cell 整簇重采样) + 剔除饱和行 | analysis.py | ✓ |
| 5 | 权重侧 off-by-one (hs[L+1] 对齐 hook) + 全层消融 + 正则检测器 | weight_common.py | ✓ |
| 6 | exp_S 随机方向充分性对照 | exp_S_v2_patching.py | ✓ |
| 7 | exp_Q 主题匹配对照 | exp_Q_v2_probe.py | ✓ |
| 8 | neutral 显式 system 字段重跑 (+502 退避重试) | api_client.py, neutral_run.py | ✓ |

## 重跑结论 (六项, 多数进一步改变了原结论)

### L2-1. corr(H,λ) 真实符号 = +0.65, 不是 -0.994 (符号反转)
- 解耦后用现有数据重算: 去掉 3 个机械饱和点 corr=-0.434 (REVISION R1 已记)。
- 显式 system + 502 重试重跑 neutral (洁净通道, 6/8 cell H 可测):
  **corr(H,λ)=+0.6492**。
- H1 预测"意图熵在拒绝前坍缩"⇒ 负相关。实测正相关, **行为级时序主张被实测
  符号反转证伪**, 不只是"不显著"。原 -0.994 是 100% 机械假象。

### L2-2. SEVERITY 效应在洁净通道稳健 (R10 排除)
- 显式 system 通道 (绕开代理 override 注入) 重跑: SEVERITY 主效应 **0.771**
  (原污染通道 0.723)。不降反升, malign cell 仍 λ=1.0。
- 严重度驱动拒绝是真效应, 不是代理注入的 cyber-pentest override 产物。

### L2-3. 全层消融把拒绝压到 0.00 且语义保持 (R12 坐实)
- exp_P_v2: baseline λ=0.99 → 单层(L16, off-by-one 已修)消融 0.20 →
  **全层消融 0.00, coherent=True**。
- 原"单层残余 ~0.12" 部分是单层干预太弱, 不是纯多方向。

### L2-4. CBRN "残余" 大部分是方法学假象 (发现12/13 实质削弱)
- exp_U_v2: CBRN 单层残余 0.16 → **全层 0.06** (< cyber 全层 0.12)。
  CBRN 全层消融后无特殊抗性。
- cos(d_CBRN, d_cyber) = **0.782** (高), **H-rank 被削弱**: CBRN 不用显著
  不同的方向。原"CBRN 残余来自独立 harmfulness 方向/多层冗余"主张, 大部分
  其实是 off-by-one + 单层太弱 + 裸 sorry 检测器三重噪声的叠加。

### L2-5. refusal direction 充分性成立 (R7 排除, 发现8 升级)
- exp_S_v2 随机方向对照: 同范数随机方向注入 harmless 催拒绝增量 **+0.06**;
  refusal direction **+0.56**; 特异性 **+0.50**。必要性 (减方向压 harmful)
  0.91→0.00。
- 排除"任意大扰动跑偏到 sorry"的替代解释。发现8 从 REVISION 的"仅必要性"
  **升级回充分且必要 (带对照)**。

### L2-6. 意图可分性真实但比原报告深 ~2 层 (R8 部分成立)
- exp_Q_v2 主题匹配对照 (同词汇翻意图, deadbolt/passport/ddos/av, n=8):
  全集 hs[12] AUC=1.0, 但**主题匹配 hs[12] 仅 0.25** ⇒ layer-12 满分确实
  主要是主题词汇可分 (R8 关切对)。
- 但**主题匹配 hs[14]+ 也跳到 1.0** ⇒ 同词汇翻意图在 hs[14] 真线性可分。
- 结论: 意图可分性真实存在, 但凝结层比原报告 (hs[12]) 深约 2 层 (hs[14])。
  n=8 LOO 满分较脆, "早层低→hs14 跳升" 的结构才是可信信号。

## 层 2 净效应

- **被证伪/反转**: H1 行为级时序主张 (corr 符号反转, +0.65)。这是项目最强的
  "宣传级"主张, 现已彻底倒掉。
- **被削弱**: 发现12/13 CBRN 残余 (大部分方法学假象); 发现10 层定位 (深 2 层)。
- **被加强**: 发现8 因果充分性 (随机对照排除替代解释); SEVERITY 效应 (洁净
  通道稳健); 全层单方向消融 (0.00 + 语义保持, 单方向中介在小模型上很干净)。
- **未变**: 合取门 S2∧S6, override 承重排序, reframing R2 穿透, 自我证伪弧线。

净: 权重侧因果机制 (单方向、可消融、可解码) 比原报告更扎实; 行为侧时序主张
(H1 的"熵坍缩领先拒绝") 被洁净数据反转证伪。**EPISTEMICS 的核心病灶"宣传强度
与证据强度反向" 在层 2 后修正——最强证据现在落在最保守的机制主张上。**
